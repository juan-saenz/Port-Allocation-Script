...
if len(sys.argv) < 3:
    ...
    for dir_name in os.listdir(basepath):
        ...
        if not os.path.isdir(dir_path):
            continue
        with open(os.path.join(basepath, dir_name+'.txt') , 'w') as outfile:
...
int_types = ["Ethernet", "FastEthernet", "GigabitEthernet", "Gigabit", "TenGigabit", "TwentyGigabit", "FortyGigabit","AppGigabitEthernet", "Serial", "ATM", "Port-channel", "Tunnel", "Loopback"]
                ...
                if m:
                    ...
                    if intinfo[name][item]['Access vlan'] == '':
                        if m.group(3) == 'trunk':
                            ...
                            intinfo[name][item]['Access vlan'] = 'trunk'
                        elif m.group(3) == 'routed':
                            intinfo[name][item]['Switchport mode'] = 'routed'
                            intinfo[name][item]['Access vlan'] = 'routed'
                        else:
